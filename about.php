<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial scale=1.0">
        <meta http-equiv="X-UA compatible" content="i.e=edge">
        <title>SEKU GUEST HOUSE | ABOUT US</title>
        <link rel="stylesheet" href="css/about.css">
        <link rel="stylesheet" type="text/css" href="CSS\bootstrap.css"/>
        <link href="http://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.3.0/css/font-awesome.css" rel="stylesheet"  type='text/css'>
	    <link href='https://fonts.googleapis.com/css?family=Open+Sans:400,300,600,700' rel='stylesheet' type='text/css'>
	
    </head>
        <!-- header content -->
	<div class="container bg-white">
            <div class="text-center " mr-5>
                
        <img src="images\Sekulogo.png" height=100px/>
                <div class="text-success py-3">
         <h4 class="h1 heading-4">SEKU GUEST HOUSE</h4>
                    </div class="bg-primary">
                </div>


    
        <!-- navigation-standard menu -->
	
		
        <nav class="navbar navbar-expand-lg navbar-dark badge-success">
            <a class="navbar-brand" href="#">
                <i class="fab fa-facebook-square"></i>
        <i class="fab fa-twitter"></i>
        <i class="fab fa-linkedin"></i>
        <i class="fab fa-google-plus-g"></i>
            </a>
            <button class="navbar-toggler" type="button" data-toggle="collapse" title="menu" data-target="#nav" aria-controls="nav" aria-expanded="false" aria-label="Toggle navigation">
              <span class="navbar-toggler-icon">
              </span>
            </button>
            <div class="collapse navbar-collapse justify-content-end"  id="nav">
              <div class="navbar-nav pr-lg" >
                <a class="nav-item nav-link" href="index.php">HOME</a>
                <a class="nav-item nav-link active" href="about.php"><span class="sr-only">(current)</span>ABOUT US</a>
                <a class="nav-item nav-link" href="meals.php">MEALS</a>
                <a class="nav-item nav-link" href="Rooms.php">ROOMS</a>
                <a class="nav-item nav-link" href="contacts.php">CONTACTS</a>
                <a class="nav-item nav-link" href="admin/adminlog.php">ADMIN</a>
                <div class="dropdown show">
                <a class="dropdown-toggle nav-item nav-link" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    USERS
                </a>
                <div class="dropdown-menu dropdown-menu-right bg-success" aria-labelledby="dropdownMenuLink">
                    <a class="dropdown-item" href="signup.php" >Sign up</a>
                    <a class="dropdown-item" href="login.php">Login</a>
                </div>
                </div>
              </div>
            </div>
          </nav>

        <div>
        
        
        
        
    <body>
        <div class="container-content bg-white">
        <div class="headtext"><h1>About us</h1>
       
        <div class="side">
                    <p> Seku Guest House is a beautiful small guest house found in The South Eastern Kenya University Permises that is in Kwa Vonza Kitui county.It is owned by the University and all the services are run the manager who is employed by the university.
                        The manager is helped by the cateress and head cook who help Her to run the services when she is away. 
                        It is 18km away from Kwa vonza market center.<br>
                    <br>SEKU guest house is just a simple hotel that mostly was built for the vice chancellor and the deputy vice chancellor but 
                    with time the vice chancellor siezed to spend their nights their after work in the university and there hence forth it was now used to provide guest house services and also
                    some rooms are rented out.<br>
                    <br>As time went by the university continued furnishing it so as to meet the standards of the 
                    esteemed guests who come for various services.
                     <br>The rooms are of various of various categories; we have the executive and normal rooms.The 
                rooms are charged differently depending on what category you need.<br>
                        
                        <br>Currently, the guest house does the following services there are meals where people come and eat the 
                        food and pay only via Mpesa.Also there are rooms for sleeping and there is also a boardroom where you can reseerve the room for 
                        meetings.<br>
                <br>In our services we ensure equality,comfortability,and satisfaction of all our customers a good place to stay
                     We offer services to every race and therefore if you are worried about the issue with race then in seku guest house that is solved.<br>
                     <br>


              </p>
                      </div>
                </div>
                <div class="carrier text-white">
                    <div id="showcase">	  
                        <p><h2>The Best Place To Stay</h2>
                    A good place to stay</p>
                </div>
            </div>
        
    </div>
        <footer>
           <div class="content justify-content-center">
	<div class="row">
	<div class="col-lg-6 col-md-6">
	
	<h5>CONTACTS</h5>
	<p><i class="fas fa-home"></i>Kwa Vonza,kitui,P.O. Box 170-90200</p>
	<p><i class="fas fa-phone-square-alt"></i>Tel:+254702598123</p>
	<p><i class="fas fa-envelope"></i>info@sekuguesthouse</p>
	</div>
<div class="col-lg-6 col-md-6">
	<h5>STAY IN TOUCH</h5>
	<i class="fab fa-facebook"></i>
	<i class="fab fa-twitter"></i>
	<i class="fab fa-snapchat"></i><br>
	<input type="email" placeholder="Subscribe for updates"><button class="btn btn-success">subscribe</button>
	</div>
	</div>
	<h2>SEKU GUEST HOUSE,COPYRIGHT &copy;2020</h2>
	</div>
	
        </footer>
        <script src="https://kit.fontawesome.com/bf257a5746.js" crossorigin="anonymous"></script>
   <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
 <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
 <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
    </body>
</html>